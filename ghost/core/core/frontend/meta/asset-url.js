...
function safeResolvePath(rootPath, relativePath) {
    ...
    const fullPath = pathModule.resolve(root, normalized);

    // Ensure the resolved path is within the root directory
    if (!fullPath.startsWith(root + pathModule.sep) && fullPath !== root) {
        return null;
    }
...
function getThemeAssetHash(assetPath) {
    ...
    const assetsRoot = pathModule.join(activeTheme.path, 'assets');
