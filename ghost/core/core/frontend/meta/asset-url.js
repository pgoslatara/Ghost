...
function getThemeAssetHash(assetPath) {
    ...
    const fullPath = pathModule.join(activeTheme.path, 'assets', assetPath);
...
function getPublicAssetHash(assetPath) {
    ...
    if (staticFilePath) {
        const staticPath = pathModule.join(staticFilePath, filename);
    ...
    if (builtFilePath) {
        const builtPath = pathModule.join(builtFilePath, filename);
