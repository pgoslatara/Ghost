...
export interface DevEnvironment {
    ...
    tinybird: {host: string; port: number};
}

export interface TestEnvironment {
    projectNamespace: string;
    gateway: {image: string};
    ghost: {image: string; workdir: string; port: number; env: string[]};
}
...
export class DevGhostManager {
    ...
    private async buildEnv(database: string = 'ghost_testing', extraConfig?: Record<string, string>): Promise<string[]> {
        const {mysql, redis, tinybird} = this.config.devEnvironment;
        
        const env = [
            ...this.config.testEnvironment.ghost.env,
            // Database connection
        ...
        if (await isTinybirdAvailable()) {
            env.push(
                `TB_HOST=http://${tinybird.host}:${tinybird.port}`,
                `TB_LOCAL_HOST=${tinybird.host}`,
                `tinybird__stats__endpoint=http://${tinybird.host}:${tinybird.port}`,
    ...
    private async createGhostContainer(
        ...
        const {ghost, projectNamespace} = this.config.testEnvironment;
        ...
        const config: ContainerCreateOptions = {
            ...
            NetworkingConfig: {
                EndpointsConfig: {
                    [this.config.devEnvironment.networkName]: {Aliases: [name]}
                }
            },
            Labels: {
                'com.docker.compose.project': projectNamespace,
    ...
    private async createGatewayContainer(name: string, ghostBackend: string): Promise<Container> {
        const {gateway, ghost, projectNamespace} = this.config.testEnvironment;
        // Gateway just needs to know where Ghost is - everything else uses defaults from the image
        const config: ContainerCreateOptions = {
            name,
            Image: gateway.image,
            Env: [`GHOST_BACKEND=${ghostBackend}:${ghost.port}`],
            ExposedPorts: {'80/tcp': {}},
            HostConfig: {
    ...
    async cleanupAllContainers(): Promise<void> {
        try {
            const containers = await this.docker.listContainers({
                all: true,
                filters: {
                    label: [`com.docker.compose.project=${this.config.testEnvironment.projectNamespace}`]
                }
            });
    ...
    async runMigrations(database: string): Promise<void> {
        ...
        const repoRoot = path.resolve(__dirname, '../../../..');
        const {mysql} = this.config.devEnvironment;
        const {ghost, projectNamespace} = this.config.testEnvironment;
        ...
        const container = await this.docker.createContainer({
            ...
            NetworkingConfig: {
                EndpointsConfig: {
                    [this.config.devEnvironment.networkName]: {}
                }
            },
            Labels: {
                'com.docker.compose.project': projectNamespace,
